...
def printNode(node):
    ...
    print('*'*7)
    global nodeNumber
    print('Düğüm:', nodeNumber)
    print('Derinlik:', len(node[9:]))
    print('Hareketler:', node[9:])
    print('-'*80)
    nodeNumber += 1
...
if __name__ == '__main__':
    ...
    while (not found and not len(nodeList)==0):
        ...
        if blankIndex!=2 and blankIndex!=5 and blankIndex!=8 and found==False:
            ...
            rightNode.append('sağ')
            found = checkFinal(rightNode)
    
    t1 = time.time()
    print('Harcanan Zaman:', t1-t0)
    print('-'*80)